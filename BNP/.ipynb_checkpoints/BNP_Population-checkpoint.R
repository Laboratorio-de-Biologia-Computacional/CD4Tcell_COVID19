...
populationSimulation <- function(net, tStates=100, simulations=100, new.nodes=c(), new.values=c(), gene=NULL, duration = 1, value = 1, pertTime=10, onePert = TRUE) {
  ...
  for(fn in new.nodes){
    if(is.na(match(fn,nodesNames))==TRUE){
      stop( paste(c('Node', fn, 'in new.nodes do not match with any gene in network'), collapse=' ') )
    }
  }
  if(length(gene)!=0){
  ...
  for(i in 1:simulations) {
    ...
    for(j in 2:(tStates+1)) {
      ...
      x <- as.vector(stateTransition(net, state = x, type = 'synchronous'))
      finalTemp[j,] <- unlist(x)
    }
    print(finalTemp)
